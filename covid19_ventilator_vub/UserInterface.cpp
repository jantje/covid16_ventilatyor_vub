...
void UserInterface::setup() {
    myScreen.setup();
    uint8_t buttonPins[sizeof(buttons)] = {
        13,
        12,
        11,
        10,
...
void UserInterface::loop() {
    ...
    myScreen.loop();
    for (unsigned int curButton = 0; curButton < sizeof(buttons); curButton++) {
        buttons[curButton].loop();
        buttonJump[curButton] = buttons[curButton].jumpValue();
    }

    requestedTriggerPressure = requestedTriggerPressure + buttonJump[7] - buttonJump[3];
    requestedBPM = requestedBPM + buttonJump[8] - buttonJump[4];
    requestedTargetVolume = requestedTargetVolume + buttonJump[9] - buttonJump[5];
